...
public class ButtonBlock2 extends WallMountedBlock {
    public static final MapCodec<ButtonBlock2> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(ModBlockSetTypes.CODEC.fieldOf("block_set_type").forGetter(b -> b.blockSetType), Codec.intRange(1, 1024).fieldOf("ticks_to_stay_pressed").forGetter(b -> b.pressTicks), ButtonBlock2.createSettingsCodec()).apply(instance, ButtonBlock2::new));
    public static final BooleanProperty POWERED = Properties.POWERED;
    private final ModBlockSetTypes blockSetType;
    private final int pressTicks;
    private final Function<BlockState, VoxelShape> shapeFunction;

    public MapCodec<ButtonBlock2> getCodec() {
        return CODEC;
    }

    public ButtonBlock2(ModBlockSetTypes blockSetType, int pressTicks, AbstractBlock.Settings settings) {
    ...
    protected int getStrongRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        if (state.get(POWERED).booleanValue() && ButtonBlock2.getDirection(state) == direction) {
            return 15;
        }
        return 0;
    }
    ...
    private void updateNeighbors(BlockState state, World world, BlockPos pos) {
        Direction direction = ButtonBlock2.getDirection(state).getOpposite();
        Direction secondary = direction.getAxis().isHorizontal() ? Direction.UP : state.get(FACING);

        WireOrientation wireOrientation = OrientationHelper.getEmissionOrientation(world, direction, secondary);
