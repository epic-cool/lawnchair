...
public abstract class LeavesSlabBlock extends SlabBlock implements Waterloggable {
    ...
    public abstract MapCodec<? extends LeavesSlabBlock> getCodec();

    public LeavesSlabBlock(float leafParticleChance, AbstractBlock.Settings settings) {
        super(settings);
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldDecay(state)) {
            LeavesSlabBlock.dropStacks(state, world, pos);
    ...
    protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        world.setBlockState(pos, LeavesSlabBlock.updateDistanceFromLogs(state, world, pos), Block.NOTIFY_ALL);
    }
    ...
    protected BlockState getStateForNeighborUpdate(BlockState state, WorldView world, ScheduledTickView tickView, BlockPos pos, Direction direction, BlockPos neighborPos, BlockState neighborState, Random random) {
        ...
        if ((i = LeavesSlabBlock.getDistanceFromLog(neighborState) + 1) != 1 || state.get(DISTANCE) != i) {
    ...
    private static BlockState updateDistanceFromLogs(BlockState state, WorldAccess world, BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            i = Math.min(i, LeavesSlabBlock.getDistanceFromLog(world.getBlockState(mutable)) + 1);
            if (i == 1) break;
        }
    ...
    private static int getDistanceFromLog(BlockState state) {
        return LeavesSlabBlock.getOptionalDistanceFromLog(state).orElse(7);
    }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        LeavesSlabBlock.spawnWaterParticle(world, pos, random, blockState, blockPos);
    ...
    private void spawnLeafParticle(World world, BlockPos pos, Random random, BlockState state, BlockPos posBelow) {
        ...
        if (LeavesSlabBlock.isFaceFullSquare(state.getCollisionShape(world, posBelow), Direction.UP)) {
            return;
        }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return LeavesSlabBlock.updateDistanceFromLogs(blockState, ctx.getWorld(), ctx.getBlockPos());
    }
}
