...
public class TntSlabBlock extends SlabBlock {
    ...
    public TntSlabBlock(Settings settings) {
        super(settings);
    ...
    protected void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        ...
        if (world.isReceivingRedstonePower(pos) && TntSlabBlock.primeTnt(world, pos)) {
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!world.isClient() && !player.getAbilities().creativeMode && state.get(UNSTABLE).booleanValue()) {
            TntSlabBlock.primeTnt(world, pos);
        }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (TntSlabBlock.primeTnt(world, pos, player)) {
    ...
    protected void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        if (world instanceof ServerWorld) {
            ...
            if (projectile.isOnFire() && projectile.canModifyAt(serverWorld, blockPos) && TntSlabBlock.primeTnt(world, blockPos, entity instanceof LivingEntity ? (LivingEntity)entity : null)) {
